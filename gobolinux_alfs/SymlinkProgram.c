...
extern int link_or_expand_all_main(int argc, char** argv);
...
do_link(char *to, char *from)
{
	...
	argv[0] = "/tools/bin/LinkOrExpandAll";
	argv[1] = from;
	argv[2] = overwrite;
	argv[3] = relative;
	argv[4] = NULL;
...
symlink_program(char *name, char *version)
{
    ...
    if (dir_exists(dirname) && strlen(goboTasks)) {
    ...
    if (dir_exists(dirname)) {
        if (! printed)
            print_msg("Symlinking executables...\n");
        do_link(goboSExecutables, dirname);
    }

    set_dir(dirname, "Environment");
    if (dir_exists(dirname) && strlen(goboEnvironment)) {
...
{
	fprintf(stderr,
			"%s: Link a program from the /Programs hierarchy in the /usr tree.\n\n"
...
main(int argc, char **argv)
{
    ...
    GET_OR_ASSIGN(goboShared,       "goboShared",       "/usr/share");
    GET_OR_ASSIGN(goboEnvironment,  "goboEnvironment",  "");
    GET_OR_ASSIGN(goboTasks,        "goboTasks",        "");
